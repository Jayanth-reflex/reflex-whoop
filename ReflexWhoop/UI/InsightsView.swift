        ...
        NavigationStack {
            Group {
                if correlations.isEmpty && anomalies.isEmpty && !isLoading {
                    ...
                    List {
                        if !findings.isEmpty {
                            Section("What moves your recovery") {
...
private struct CorrelationCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(sentence)
                .font(.subheadline)
            HStack(spacing: 6) {
                chip(String(format: "rho %+.2f", row.rho))
                chip("n = \(row.n)")
                chip(row.strengthEnum.rawValue.capitalized)
                if let p = row.pValueBhCorrected {
                    chip(String(format: "p(adj) %.2f", p))
                }
            }
        }
        .padding(.vertical, 4)
    ...
    private var sentence: String {
        ...
        return "Higher \(name.prefix(1).lowercased() + name.dropFirst()) tends to come with \(direction) next-day recovery."
    }
...
private struct AnomalyTimelineRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(anomaly.day).font(.caption).foregroundStyle(.secondary)
            Text(text)
        }
    }
    ...
    private var text: String {
        if anomaly.kind == "illness_flag" {
            return "Multiple signals (respiratory rate, skin temp, resting HR, HRV) moved together — possible early illness signal."
        }
        if let metric = anomaly.metric, let z = anomaly.zScore {
            ...
            return "\(label) unusually \(direction) (z = \(String(format: "%.1f", z)))."
        }
